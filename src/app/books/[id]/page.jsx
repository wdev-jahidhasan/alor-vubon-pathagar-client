...
export default function BookDetailsPage({ params }) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 space-y-4">
        ...
        >
          ...
          <Link
            href="/books"
            className="px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-semibold text-xs sm:text-sm rounded-xl transition-colors inline-flex items-center gap-2 shadow-lg shadow-purple-600/30"
          >
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white pt-6 sm:pt-10 pb-16 px-3 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto space-y-6 sm:space-y-8 relative z-10">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handleShare}
              className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white rounded-xl text-xs font-medium transition-colors flex items-center gap-1.5 cursor-pointer shadow-sm"
            >
              {copied ? <Check size={14} className="text-emerald-400" /> : <Share2 size={14} />}
              {copied ? 'লিংক কপি হয়েছে!' : 'শেয়ার করুন'}
            </button>

            <div className="text-[11px] sm:text-xs text-slate-300 hidden md:block">
        ...
        <motion.div
          ...
          className="bg-slate-900/60 border border-slate-800/80 rounded-2xl sm:rounded-3xl p-4 sm:p-8 lg:p-10 grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8 items-start shadow-2xl backdrop-blur-md"
        >

          {/* ইমেজ সেকশন */}
          <div className="md:col-span-5 relative rounded-2xl overflow-hidden border border-slate-800/80 shadow-2xl h-72 sm:h-96 md:h-[420px] group">
            <img
              src={book.image}
              alt={book.title}
          ...
          <div className="md:col-span-7 space-y-4 sm:space-y-6 flex flex-col justify-between">
            <div className="space-y-3">
              <span className="inline-block bg-purple-950/80 border border-purple-800/50 text-purple-300 text-xs sm:text-sm font-semibold px-3 py-1 rounded-full shadow-inner">
                {book.category}
              </span>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              {reviews.length > 0 && (
                <button
                  ...
                  className="px-4 py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-semibold text-xs sm:text-sm rounded-xl transition-all duration-300 flex items-center gap-2 shadow-lg shadow-purple-600/30 hover:scale-[1.02] cursor-pointer"
                >
            ...
            <div className="bg-slate-950/80 border border-slate-800/80 rounded-2xl p-4 space-y-3 shadow-inner">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <a
                  ...
                  className="px-4 py-2 bg-purple-600/90 hover:bg-purple-600 text-white font-semibold text-xs sm:text-sm rounded-xl transition-colors flex items-center gap-2 shadow-md shadow-purple-600/20"
                >
                  <Phone size={14} /> কল করুন: +৮৮০ ১XXXXXXXXX
                </a>
                <Link
                  href="/books"
                  className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold text-xs sm:text-sm rounded-xl transition-colors flex items-center gap-2 border border-slate-700/60"
                >
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4">
            <motion.div
              ...
              className="bg-slate-900 border border-slate-800 rounded-3xl max-w-2xl w-full p-5 sm:p-7 space-y-5 relative max-h-[85vh] overflow-y-auto shadow-2xl"
            >
              ...
              <div className="space-y-4">
                {reviews.map((rev, idx) => (
                  <div key={idx} className="bg-slate-950/70 p-4 sm:p-5 rounded-2xl border border-slate-800/80 space-y-2 shadow-sm">
                    <div className="flex justify-between items-center">
                      <h4 className="font-semibold text-white text-sm sm:text-base">{rev.reviewer}</h4>
                      <span className="text-[11px] text-purple-300 bg-purple-950/60 px-2.5 py-0.5 rounded-md border border-purple-800/40">
                        {rev.reviewerDetails}
                      </span>
                    </div>
